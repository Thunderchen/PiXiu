...
原计划是接下来学习Linux C API，撸一个协程网络库。再然后，加上分布式。真这么玩下去，要猝死在电脑前了。尽管，这是一个未真正完成的项目。但最独特的内核，我已经保质保量地完成了。所以，我现在发布的是一个没有**任何耦合**的索引引擎。
...
3. 使用Xcode内置profiler修复干净了内存泄漏和越界。（Valgrind在最新的OSX下跑不起，Linux各种还在学习中）
